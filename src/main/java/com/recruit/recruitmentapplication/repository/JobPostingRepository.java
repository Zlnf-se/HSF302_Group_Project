...
public interface JobPostingRepository extends JpaRepository<JobPosting, Long> {
    ...
    @Query("""
            SELECT jp FROM JobPosting jp JOIN FETCH jp.company c
            WHERE jp.status = 'OPEN'
              AND (:keyword = '' OR LOWER(jp.title) LIKE LOWER(CONCAT('%', :keyword, '%')))
              AND (:department = '' OR c.industry = :department)
              AND (:location = '' OR jp.location = :location)
            ORDER BY jp.postedDate DESC
            """)
    List<JobPosting> findOpenJobsWithCompanyAndFilters(
            @Param("keyword") String keyword,
            @Param("department") String department,
            @Param("location") String location
    );

    @Query("SELECT DISTINCT c.industry FROM JobPosting jp JOIN jp.company c WHERE jp.status = 'OPEN' AND c.industry IS NOT NULL ORDER BY c.industry")
    List<String> findOpenDepartments();

    @Query("SELECT DISTINCT jp.location FROM JobPosting jp WHERE jp.status = 'OPEN' AND jp.location IS NOT NULL ORDER BY jp.location")
    List<String> findOpenLocations();
