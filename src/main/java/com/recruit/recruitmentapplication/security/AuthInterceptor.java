...
public class AuthInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
        ...
        if (isPublicJobReadRequest(request)) {
            return true;
        }
        ...
        if (isCandidatePortalRequest(request) && !Role.CANDIDATE.equals(loggedInUser.getRoleName())) {
            response.sendRedirect(request.getContextPath() + "/error/403");
            return false;
        }
    ...
    private boolean isJobWriteRequest(HttpServletRequest request) {
        ...
        if (path.startsWith("/jobs/") && path.endsWith("/apply")) return false;
    ...
    private boolean isPublicJobReadRequest(HttpServletRequest request) {
        ...
        if ("/jobs".equals(path)) return true;
        return path.matches("/jobs/\\d+");
    }

    private boolean isCandidatePortalRequest(HttpServletRequest request) {
        String path = request.getRequestURI().substring(request.getContextPath().length());
        if (path.startsWith("/applications/my")) return true;
        if (path.startsWith("/applications/") && path.endsWith("/withdraw")) return true;
        return path.startsWith("/jobs/") && path.endsWith("/apply");
    }
